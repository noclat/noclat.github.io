...
<h1
  class={cx(
    ...
    'font-black uppercase text-gray-950',
    $$props.class
  )}
>
  {#each title.toLocaleUpperCase().split(/[\s\n\r]+/) as line, i (i)}
    <span class="flex flex-col justify-end h-[var(--unit)] overflow-hidden bg-gray-950">
      <span class="inline-flex items-baseline relative top-[.2em] xl:top-[.23em]">
        {#each line.split('') as char, j (j)}
          <span
            ...
            class={cx(
              'animate-type-in after-[var(--delay)] transform-gpu text-gray-100',
              {
